// initial data
let section = 'page-1'; // pagina atual
let currentPage = parseInt(section.slice(5)); // numero da pagina atual
...
function printCard() {
    ...
    moviesJson.map((item) => {
        ...
        if(item.page == section && clonesQuantity < maxCards ) { // verifica se o item no objeto pertence a pagina e delimita a quantidade de impressão
            ...
            cardItem.classList.add(`${item.page}`); // insere o id que identifica o numero da pagina a que pertence
            ...
            cardItem.querySelector('.clone .btn').setAttribute('data-index', item.id); // insere o id do objeto p/ uso no form.html
        }         
    });
    let cloneBtns = document.querySelectorAll('.clone .btn'); // array de botoes avaliar
    // ao clicar em avaliar insere o id do filme do card clicado no html e salva no navegador p/ usar no form.html
    cloneBtns.forEach(e => e.addEventListener('click', () => { // p/ cada item adiciona o evento de click
        itemId =  e.getAttribute('data-index'); // salva em itemId o id do objeto do card clonado
        localStorage.setItem("id", itemId); // salva localmente no navegador o itemId
    }));
   ...
   // no array de itens da paginação adiciona evento de click em cada item 
    pageItem.forEach(e => e.addEventListener('click', () => { 
        if(e.id == 'next' && currentPage < pageEnum.length) { // se ao clicar no botao next não for a ultima pagina
            section = `page-${currentPage += 1}`; // altera o numero da pagina para a proxima
           ...
           activeClass(pageEnum, pageItem[currentPage]); // muda a classe active do elemento atual para o proximo
            ...
            section = `page-${currentPage}`; // altera o numero da pagina de acordo com o numero clicado
    ...
    function eraseCard() {
        ...
        clones.forEach(e => { // p cada item no array de clones
            if(!e.classList.contains(`${section}`)) { // se o item não contem a classe pagina(se ele não pertence a pagina)
                e.remove(); // remova-o
            }
        });
    }
